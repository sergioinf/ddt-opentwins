...
def save_result_json2(img, pred):
    img_h, img_w, _ = img.shape
    results = []
    pred = pred[pred[:, 0].astype(int) == 1]
...
def recognize_from_image(net, orig_target_sizes):
    ...
    for image_path in args.input:
        ...
        #To get real results
        resultados = save_result_json2(image, results[0])
        print(resultados)
...
def on_message(client, userdata, message):
    url = "http://192.168.48.206:30525/api/2/things/distributed:imagerecognition/inbox/messages/move"
    msg = json.loads(message.payload.decode("utf-8"))
    msg = msg['value']
    
    image = msg['image']['properties']['value'][2:]
    msg.pop('image')
...
if __name__ == '__main__':
    ...
    client = mqtt.Client("ResendConsumer")
    client.connect(mqttBroker, port=) 
    client.subscribe("opentwins/twin/events/distributed/imagerecognition")
